...
if(isset($_REQUEST['login']))  //name
{
  ...
  $cpass=$_REQUEST['confirmpass'];
   $lgn="select * from teacher where user='$user' AND aadhar_no='$aadhar'"; //agar match karegi values AND  returns 1 when both same
  #exit();
  ...
  if($chk==1)  //IF INSERT get success then ...
  {
    ...
    if($pass==$cpass)
   {
   $up="update teacher set pass='$pass' where user='$user'";
